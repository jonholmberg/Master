...
\begin{theorem}
 If $p$ is an odd prime \todo{move?} then
 \[
...
\end{tikzcd}
\]
Hence the topological generator $g^{p-1}$ is sent to a topological generator of $p\Z_p$. Such a generator is of the form $pu$ where $u$ is a $p$-adic unit. Now write $\frac{n}{p-1} = tp^{v_p(n)}$ \todo{check} where $t$ is a $p$-adic unit. Then $\log(g^n) = \log(g^{p^{v_p(n)}(p-1)t}) = utp^{v_p(n)+1}$ and since $u$ and $t$ both are $p$-adic units, so is $ut$ and hence $\log(g^n)$ generates the subgroup $p^{v_p(n)+1}\Z_p$.
Thus $g^n$ generates $(1 + p^{v_p(n)+1}\Z_p)^*$ and so \todo{??} $g^{n}-1$ generates $(p^{v_p(n)+1}\Z_p)^*$ and we can write $g^n-1 =u' p^{v_p(n)+1}$ for a $p$-adic unit $u'$ so that
\iffalse
...
\begin{theorem}
  The homotopy groups of $E(1)^{h\Z_2^*}$ are given by \todo{check+weights}
  \[
  \pi_nE(1)^{h\Z_2^*} \simeq
    \begin{cases}
      \Z_2, & n = -1 \\
      \Z/2 \times \Z_2, & n = 0 \\
      \Z / 2, & n \equiv 0 \text{ mod } 8 \text{ and } n \neq 0 \\
      \Z / 2 \times \Z / 2, & n \equiv 1 \text{ mod } 8 \\
      \Z / 2, & n \equiv 2 \text{ mod } 8 \\
      \Z / 8, & n \equiv 3 \text{ mod } 8 \\
      \Z / 2^{v_2(n) + 1}, & n \equiv 7 \text{ mod } 8 \text{ and } n \neq -1 \\
      0, & \text{otherwise.}
    \end{cases}
  \]
\end{theorem}
...
\end{tabular}
\end{table}
Notice that there are no non-trivial differentials on page two and so $E^2 = E^3$ but on the $E^3$ page there are possible non-trivial differentials. Let $u \in H^0(B\{\pm 1\}, \pi_4E(1)) \cong \Z_2(2)$ and $t \in H^1(B\{\pm 1\}, \pi_2E(1)) \cong \Z/2$ denote generators. Notice that $2t=0$
and since we have non-trivial homotopy groups of $E(1)$
in the even negative degrees, $u$ is invertible, and so
$H^*(B\{\pm 1\}, \Z_2)$ is the algebra $\Z_2[t, u^{\pm}]/(2t)$.

We now look at the differentials on the $E^3$-page, and as it turns out, most of them disappear. First notice that $E(1)$ is an $E_\infty$-ring spectrum, and so there is a map $\Sph \to E(1)$ since $\Sph$ is initial among $E_\infty$-ring spectra. It is a non-trivial fact that on homotopy groups this maps sends the Hopf map $\eta$ to our generator $t$ \todo{??}, and since we know that $\eta^4 \in \pi_4(\Sph) = 0$ we must have that $\eta^4 = 0$ and so $t^4 = 0$. Hence the differential on $E^3$ satisfy that $0 = d^3 t^4$
which then forces $d^3t = 0$ by the multiplicative structure on the differentials. Using this we see that $0 = t^4 = d^3(tu)=td^3(u)$, and since $t$ is not a zero divisor \todo{why}, this implies that $d^3(u) = t^3$. The multiplicative structure of the differentials then tells us how all the differentials on the $E^3$-page looks, and there are no non-trivial differentials on $E^k$ for $k \ge 4$ so $E^4 = E^\infty$ \todo{check this}, and we can calculate the desired homotopy group from the differentials on the page $E^3$. We notice that the kernel of the map
\[
d^3 \colon \Z_2(2)[u] \to \Z/2(3)[t^3]
\]
is generated by $2u$ since $d^3(2u)=2t^3 = 0$, and $\Z_2(2)[u]$ sits in total degree $4$, and the other groups in total degree $4$ vanishes, hence we get the desired conclusion. \todo{elaborate a bit why everything vanishes}
\end{proof}
...
\begin{enumerate}[i)]
  \item When $i=0$, $1-5^0 = 0$ so it has kernel and cokernel $\Z_2(0)$. When $i \equiv 0$ mod $8$ and $i \neq 0$, $1-5^{1/2}$ the map is injective and the cokernel is given by $\Z/2^{v_2(1-5^{i/2})}\Z$. By Adams Lemma 2.12 \todo{ref} we have that $v_2(1-t^{i/2}) = 2 + v_2(i/2) = 1 + v_2(i)$,
  ...
  \item This case requires a bit of work. We have seen that the cokernel of $1-t^{i/2}$ is $\Z/2^{v_2(1-t^{i/2})}\Z$, and so we calculate $v_2({1-t^{i/2}})$ when $i \equiv 4$ mod $8$, i.e. $i/2 \equiv 2$ mod $8$ \todo{check}. From Adams \todo{ref} Lemma 2.12 we get that $v_2(1-t^{i/2}) = 2 + v_2(i/2)$, and clearly $v_2(i/2) = 1$
  hence $v_2(1-t^{i/2}) = 3$. Thus $1-t^{i/2}$ is an injective map with cokernel $\Z/2^3 \cong \Z/8$
\end{enumerate}
From this we see that the $E^2$-page is $8$-periodic and will look like the following \todo{finish}
\begin{table}[h]
  \centering
  \setlength{\tabcolsep}{12pt}
  \setlength{\extrarowheight}{2pt}
...
\[
\pi_n \Spp_2 \to \pi_nE(1)^{h\Z_2^*}
\]
where $\Spp_2$ is the $2$-local sphere spectrum.
\todo{add weights to all lemmas + write about weight in remark}
