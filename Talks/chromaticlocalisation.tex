...
\end{definition}
Denote the Verdier quotient $\Spp/\Spp^{\ge h+1}$ by $\Spp^{\le h}$ and so we have a Verdier sequence
\[
...
\begin{remark}
  ...
  An element $u$ of $\Z_p$ is a unit, if and only if, $u$ is not divisible by $p$, i.e. $a_0 \neq 0$. We can define the \textit{$p$-adic valuation} function $v_p \colon \Z_p \to \Z \cup \{\infty\}$ as follows. Every $p$-adic integer $x \in \Z_p$ can be uniquely written as $x = up^n$ where $u$ is a $p$-adic unit and $n$ is a non-negative integer. Set $v_p(x) = n$, with the convention that $v_p(0) = \infty$.
\end{remark}
