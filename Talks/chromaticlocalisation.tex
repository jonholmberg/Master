...
\end{tikzcd}
\]
and so we study $\Z_p/{p^{v_p(g^n-1)}\Z_p}$, and in particular $v_p(g^n-1)$. As we saw earlier, there is a short exact sequence
\[
\begin{tikzcd}
1 \rar & (1+p\Z_p)^* \rar{} & \Z_p^* \rar{} & \F_p^* \rar & 1
\end{tikzcd}
...
\begin{center}
  ...
  \matrix (m) [matrix of math nodes,
    ...
    1  &     0     &  \Z  & \Z &  0  & \\
    1  & 1  &   0  &  1  &  & \\};
\draw[thick] (m-1-5.west) -- (m-4-5.west) ;
\draw[thick] (m-4-1.north) -- (m-4-4.north) ;
\end{tikzpicture}
\end{center}
